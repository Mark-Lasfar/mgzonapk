...
export default function SellerSubscriptionPage({ params: { locale } }: SellerSubscriptionPageProps) {
  ...
  const hasRedirected = useRef(false); // Track if redirection has occurred
  ...
  useEffect(() => {
    console.log("useEffect triggered", { locale, validLocale, status, session: !!session });

    // Handle locale redirection
    if (locale !== validLocale && !hasRedirected.current) {
      console.log("Redirecting to valid locale", { from: locale, to: validLocale });
    ...
    async function initialize() {
      console.log("Initializing with status:", status);
      ...
      if (status === "unauthenticated") {
        console.log("Unauthenticated, redirecting to sign-in");
        router.push(`/${validLocale}/sign-in`);
        return;
      }

      if (session?.user?.id) {
        console.log("Fetching seller data for user:", session.user.id);
        try {
          const sellerResult = await getSellerByUserId(session.user.id, validLocale);
          console.log("Seller result:", sellerResult);

          if (!sellerResult.success || !sellerResult.data) {
            console.error("Failed to fetch seller:", sellerResult.error);
    ...
    initialize();
  }, [session, status, validLocale, router, t]); // Removed pathname dependency
  ...
  async function onSubmit(data: z.infer<typeof formSchema>) {
    ...
    try {
      console.log("Submitting subscription update:", data);
