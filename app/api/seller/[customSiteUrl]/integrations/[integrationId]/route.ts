...
export async function POST(
  req: NextRequest,
  { params }: { params: { customSiteUrl: string; integrationId: string } }
) {
  const requestId = uuidv4();
  const t = await getTranslations('seller.integrations');
  let seller: ISeller | null = null; // تعريف seller في نطاق أعلى
  let integrationId: string | null = null; // تعريف integrationId في نطاق أعلى

  try {
    const { customSiteUrl, integrationId: paramIntegrationId } = params;
    integrationId = paramIntegrationId; // تعيين integrationId

    if (!mongoose.Types.ObjectId.isValid(integrationId)) {
      return NextResponse.json({ error: t('invalid_integration_id') }, { status: 400 });
    }

    await connectToDatabase();

    seller = await Seller.findOne({ customSiteUrl });
    if (!seller) {
    ...
    const sellerIntegration = await SellerIntegration.findOne({
      ...
      integrationId,
      sandbox: req.nextUrl.searchParams.get('sandbox') === 'true',
    });
    ...
    logger.info('Webhook processed', { requestId, sellerId: seller._id.toString(), integrationId });
    return NextResponse.json({ success: true });
  } catch (error) {
    const errorMessage = error instanceof Error ? error.message : String(error);
    logger.error('Failed to process webhook', {
      requestId,
      sellerId: seller?._id?.toString() || 'unknown',
