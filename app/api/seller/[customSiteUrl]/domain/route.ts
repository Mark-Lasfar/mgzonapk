...
export async function POST(req: NextRequest, { params }: { params: { customSiteUrl: string } }) {
  try {
    ...
    const store = await Store.findOne({ sellerId: params.customSiteUrl, isActive: true });
    if (!store) {
