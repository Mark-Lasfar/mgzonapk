...
export async function POST(
  ...
  try {
    const { id: providerSlug } = params; // تعريف providerSlug من params
    const { searchParams } = new URL(req.url);
    const sandbox = searchParams.get('sandbox') === 'true';

    await connectToDatabase(sandbox ? 'sandbox' : 'live');

    // البحث عن التكامل بناءً على providerName (slug)
    const integration = await Integration.findOne({
      providerName: { $regex: `^${providerSlug.replace(/-/g, ' ')}$`, $options: 'i' },
      isActive: true,
      sandbox,
    });
    ...
    logger.info('Integration webhook processed', {
      requestId,
      integrationId: integration._id,
    ...
    const providerSlug = params.id || 'unknown'; // تهيئة providerSlug بقيمة افتراضية إذا لم يكن موجودًا
    logger.error('Failed to process integration webhook', {
      requestId,
      providerSlug,
      error: errorMessage,
    });
